...
The frontend SHALL use Biome for linting and formatting, configured via `biome.json` in the `frontend/` directory. The configuration SHALL enable recommended rules.

#### Scenario: Biome check passes on clean project
- **WHEN** a developer runs `pnpm biome check .` from the `frontend/` directory
- **THEN** no linting or formatting errors are reported

#### Scenario: Biome detects formatting issues
- **WHEN** a file has incorrect formatting
- **THEN** `pnpm biome check .` reports the issue
